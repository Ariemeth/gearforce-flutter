...
void main() {
  test('test creating a unit with a test unit', () {
    ...
    expect(testUnit.attribute(UnitAttribute.name), equals(testCore.name),
        reason: 'compare name');
    expect(testUnit.attribute(UnitAttribute.ew), equals(testCore.ew),
        reason: 'compare ew');
  });
  ...
  test('test attribute function with no mods', () {
    ...
    expect(tu.attribute(UnitAttribute.name), equals(uc.name),
        reason: 'Name check');
    expect(tu.attribute(UnitAttribute.tv), equals(uc.tv), reason: 'TV check');
    expect(tu.attribute(UnitAttribute.roles), equals(uc.role),
        reason: 'Role check');
    expect(tu.attribute(UnitAttribute.movement), equals(uc.movement),
        reason: 'Movement type check');
    expect(tu.attribute(UnitAttribute.armor), equals(uc.armor),
        reason: 'Armor check');
    expect(tu.attribute(UnitAttribute.hull), equals(uc.hull),
        reason: 'Hull check');
    expect(tu.attribute(UnitAttribute.structure), equals(uc.structure),
        reason: 'Structure check');
    expect(tu.attribute(UnitAttribute.actions), equals(uc.actions),
        reason: 'Actions check');
    expect(tu.attribute(UnitAttribute.gunnery), equals(uc.gunnery),
        reason: 'Gunnery check');
    expect(tu.attribute(UnitAttribute.piloting), equals(uc.piloting),
        reason: 'Piloting check');
    expect(tu.attribute(UnitAttribute.ew), equals(uc.ew), reason: 'EW check');
    expect(tu.attribute(UnitAttribute.react_weapons), equals(uc.reactWeapons),
        reason: 'React Weapons check');
    expect(
        tu.attribute(UnitAttribute.mounted_weapons), equals(uc.mountedWeapons),
        reason: 'Mounted Weapons check');
    expect(tu.attribute(UnitAttribute.traits), equals(uc.traits),
        reason: 'Traits check');
    expect(tu.attribute(UnitAttribute.type), equals(uc.type),
        reason: 'Unit type check');
    expect(tu.attribute(UnitAttribute.height), equals(uc.height),
        reason: 'Height check');
  });
  ...
  test('test attribute function with 1 mod affecting 1 attributes', () {
    ...
    expect(tu.attribute(UnitAttribute.name), equals('${uc.name} upgrade'),
        reason: 'name check');
  });
  ...
  test('test attribute function with 1 mod affecting 2 attributes', () {
    ...
    expect(tu.attribute(UnitAttribute.gunnery), equals(uc.gunnery! - 1),
        reason: 'Gunnery check');
  });
  ...
  test('test add 2 mods with same change', () {
    ...
    expect(tu.attribute(UnitAttribute.tv), equals(uc.tv + 2),
