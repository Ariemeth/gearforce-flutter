...
const _defaultBaseUrl = 'https://hg.metadiversions.com';
const _gameStorageUrl = 'https://gs.metadiversions.com/gf';
...
class ApiService {
  ...
  static Future<UnitRoster?> getV3Roster(
    ...
    final response = await http.get(Uri.parse('$_gameStorageUrl/${id.trim()}'));
  ...
  static Future<(String?, String?)> saveV3Roster(UnitRoster roster) async {
    ...
    final response = await http.post(
      Uri.parse('$_gameStorageUrl/store/'),
  ...
  static Future<String?> getLatestVersion(Uri baseUri) async {
    final uri = Uri.parse('$_defaultBaseUrl/version.json');
